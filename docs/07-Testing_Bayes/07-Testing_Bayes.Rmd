...
- A *colleague* claims that this proportion is **over 1/3**, and I want to test how plausible this claim is. 
...
.pull-right[
I think $p$ is over 40% but I'm not very confident in that estimate. Suppose we use the following hierarchy:

- **Prior**: $p\sim Beta(8, 12)$

- **Data**: $Y\mid p\sim Binomial(n=30, p)$, where we observe $Y = 12$
]

Using the *Beta-Binomial conjugacy*, our **posterior** distribution is $$p\mid Y\sim Beta(8+12=\mathbf{20}, 12+30-12=\mathbf{30})$$

---

# Is the "over 1/3" claim plausible?
...
plot_beta(20, 30) + 
  labs(x = "p", y = "f(p)", title = "p | Y ~ Beta(18, 32)")
```
]

```{r}
# 95% Credible Interval
qbeta(c(0.025, 0.975), 20, 30)
```
...
While the **visual** of the posterior PDF and the 95% **credible interval** help, we can calculate the **posterior probability** of $p$ being *over 1/3*:

.center[
```{r, echo = FALSE, fig.height = 4, fig.width = 6, dpi = 300, out.width = "70%"}
plot_beta(20, 30) + 
  labs(x = "p", y = "f(p)", title = "p | Y ~ Beta(18, 32)", 
       subtitle = "Pink: P(p > 1/3 | Y = 10)") + 
  stat_function(fun = dbeta, 
                args = list(shape1 = 20, shape2 = 30),
                xlim = c(1/3, 1),
                geom = "area", 
                fill = "hotpink")
```
...
- **Null Hypothesis**, $H_{0}:p\leq 0.33$ (*p* is *at most* 33%)

- **Alternative Hypothesis**, $H_{A}:p>0.33$

--

We just calculated the posterior probability of the *alternative hypothesis*: $$P(H_{A}\mid Y=12)=0.832$$

- Therefore, the posterior probability of the *null hypothesis* is just the **complement**! $$P(H_{0}\mid Y=12)=1-0.832=0.168$$

---

# Posterior and Prior Odds

We can put these two probabilities together to form the **posterior odds**: $$\text{posterior odds}=\frac{P(H_{A}\mid Y=12)}{P(H_{0}\mid Y=12)}=\frac{0.832}{0.168}=\boxed{4.952}$$

- Our *posterior assessment* is that $p$ is 4.952 times more likely to be *above 1/3* than to be *below 1/3*. 

--

What about the odds **prior** to observing data?? Let's calculate **prior odds**. 

- **Recall**: Our *prior* for $p$ was $p\sim Beta(8, 12)$. Then $P(H_{A})=P(p>1/3)$:
...
$$\text{prior odds}=\frac{P(H_{A})}{P(H_{0})}=\frac{0.721}{1-0.721}=\boxed{2.584}$$
...
```{r}
# Bayes Factor
4.952/2.584 # posterior_odds / prior_odds
```
...
While the **posterior probability** of $H_{A}$, $P(H_{A}\mid Y=12)=0.832$, is quite high, the BF $<1$ established that the plausibility of my colleague's claim has *decreased* in light of the observed data. 
...
(In either case, the sample proportion is 0.4...)
...
However, when we try to calculate the **posterior probability** that $H_{0}$ is true, $P(H_{0}\mid Y=12)$, we get zero! $$P(p=0.33\mid Y=12)=\int_{0.33}^{0.33}f(p\mid y=12)\, dp=0$$
...
Recall that a **95% credible interval** for $p$ is $(0.234, 0.496)$. 

```{r}
# 95% Credible Interval
qbeta(c(0.025, 0.975), 18, 32)
```
...
With this **buffer** in place, we can more rigorously claim *uncertainty* in the plausbility of $H_{A}$.

- The hypothesized range of $p$, $(0.45, 0.55)$, overlaps with the credible interval, $(0.234, 0.496)$. 
