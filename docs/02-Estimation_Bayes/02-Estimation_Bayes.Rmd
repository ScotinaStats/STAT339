...
- Equivalently, there is a ~**89%** *posterior probability* that an article is *fake*, given the presence of an exclamation point in the title. 8
...
plot_binomial_likelihood(y = 12, n = 30) + 
  labs(x = expression(theta), y = expression(paste("P(",Y==30, " | ", theta, ")")))
```

- $P(Y=30\mid\theta=0.4)=\binom{30}{12}0.4^{12}(1-0.4)^{18}\approx 0.147$
...
gf_line(f_y(y) ~ y, size = 2) + 
  labs(x = expression(theta), 
       y = expression(paste("f(", theta, ")")), 
       title = expression(paste("f(", theta, ") = (3/2)", y^2 + y))) 
```
]
...
Let's stick with $\lambda\sim Gamma(20, 33)$. 
